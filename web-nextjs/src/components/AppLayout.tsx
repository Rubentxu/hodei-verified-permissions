...
export default function AppLayout({ children }: PropsWithChildren) {
  ...
  return (
    <div className="flex h-screen bg-gray-50 text-gray-900">
      <aside
        className={`${open ? "w-64" : "w-16"} bg-white border-r border-gray-200 transition-all duration-300 ease-in-out flex flex-col`}
      >
          ...
          <ul className="space-y-1">
            {nav.map((item) => (
              <li key={item.href}>
                <Link
                  href={item.href}
                  className={`w-full flex items-center ${open ? "space-x-3 justify-start" : "justify-center"} p-2 rounded-md transition-colors text-sm ${active(item.href) ? "bg-blue-50 text-blue-700 border-l-4 border-blue-700" : "text-gray-600 hover:bg-gray-50 hover:text-gray-900"}`}
                >
                  {item.icon}
        ...
        <div className="p-3 text-xs text-gray-500 border-t border-gray-200">
          {open ? <p>v1.0.0</p> : <span className="block text-center">v1</span>}
        </div>
      </aside>
