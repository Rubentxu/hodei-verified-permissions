...
test.describe('Playground - Fase 3 Features', () => {
  ...
  test('should configure and run single test', async ({ page }) => {
    ...
    const principalType = page.locator('input').filter({ hasText: 'User' }).first();
    const principalId = page.locator('input').filter({ hasText: 'alice' }).first();
    ...
    // Configure action
    const actionType = page.locator('input').filter({ hasText: 'Action' }).first();
    const actionId = page.locator('input').filter({ hasText: 'viewDocument' }).first();
    ...
    // Configure resource
    const resourceType = page.locator('input').filter({ hasText: 'Document' }).first();
    const resourceId = page.locator('input').filter({ hasText: 'doc123' }).first();
  ...
  test('should load saved scenario', async ({ page }) => {
    ...
    const loadButtons = page.locator('button').filter({ has: page.locator('svg') });
    await loadButtons.first().click();

    // Verify configuration was loaded
    await expect(page.locator('#scenario-name')).toHaveValue('Load Test Scenario');
  });
  ...
  test('should enable and use debug mode', async ({ page }) => {
    ...
    await expect(page.locator('text=Parse authorization request')).toBeVisible();
  });
  ...
  test('should expand debug step details', async ({ page }) => {
    ...
    const firstStep = page.locator('text=Step 1').locator('xpath=./ancestor::*[contains(@class, "border")]');
  ...
  test('should handle test errors gracefully', async ({ page }) => {
    ...
    const hasResults = await page.locator('text=Decision:').count() > 0;
    const hasTestResults = await page.locator('text=Test Results').count() > 0;
