...
test.describe('Scenarios Management', () => {
  ...
  test.beforeEach(async ({ page }) => {
    ...
    // Clear API scenarios store
    await fetch('http://localhost:3001/api/scenarios/clear', {
      method: 'POST',
    });
  });
  ...
  test('should save new scenario with all fields', async ({ page }) => {
    ...
    await expect(scenarioCards.filter({ hasText: 'Complete Scenario Test' })).toHaveCount(1);

    // Verify details are shown
    const completeScenarioCard = scenarioCards.filter({ hasText: 'Complete Scenario Test' }).first();
  ...
  test('should load scenario from saved list', async ({ page }) => {
    ...
    const scenarioCards = page.locator('[data-testid="scenario-card"]');
    const loadButton = scenarioCards.first().locator('button[aria-label="Load"]');
  ...
  test.skip('should delete scenario from saved list', async ({ page }) => {
    ...
    const scenarioCards = page.locator('[data-testid="scenario-card"]');
    await expect(scenarioCards.filter({ hasText: scenarioName })).toHaveCount(1);

    // Delete scenario (delete the most recent one which should be our test scenario)
    const testScenarioCard = scenarioCards.filter({ hasText: scenarioName }).first();
    const deleteButton = testScenarioCard.locator('button[aria-label="Delete"]');
    ...
    await expect(updatedScenarioCards.filter({ hasText: scenarioName })).toHaveCount(0);
  });
  ...
  test('should show scenario timestamps', async ({ page }) => {
    ...
    const hasTimestamp = await cardContent.locator('text=/ago|minutes|hours/').count() > 0;
