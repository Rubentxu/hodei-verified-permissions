...
test.describe('Snapshot Management - E2E Tests', () => {
  ...
  test('SN-002: Create Multiple Snapshots', async ({ page }) => {
    ...
    await page.click('button:has-text("Create Snapshot")');
    await page.fill('input[placeholder*="description"]', 'Snapshot 2 - After Changes');
    await page.click('button:has-text("Create Snapshot")');
    await page.waitForTimeout(1000);

    // Create third snapshot
    await page.click('button:has-text("Create Snapshot")');
    await page.fill('input[placeholder*="description"]', 'Snapshot 3 - Final State');
    await page.click('button:has-text("Create Snapshot")');
    await page.waitForTimeout(1000);
  ...
  test('SN-013: Snapshot Sorting (Newest First)', async ({ page }) => {
    ...
    const snapshots = page.locator('[class*="card"]').all();
    const snapshotTexts = await Promise.all(snapshots.map(s => s.textContent()));
    // Newest should appear before oldest
    const thirdIndex = snapshotTexts.findIndex(t => t?.includes('Third Created'));
    const firstIndex = snapshotTexts.findIndex(t => t?.includes('First Created'));
  ...
  test('SN-041: Snapshot with Special Characters in Description', async ({ page }) => {
  ...
  async function createTestPolicyStore(page: Page): Promise<void> {
    ...
    await page.fill('textarea[placeholder*="description"]', 'Snapshot Test Store');
  ...
  async function cleanupTestPolicyStore(page: Page): Promise<void> {
    ...
    if (await storeCard.isVisible()) {
      ...
      const deleteButton = storeCard.locator('xpath=../../..//button[@title="Delete policy store"]');
  ...
  async function createSnapshot(page: Page, description: string): Promise<void> {
    // Click Create Snapshot
    await page.click('button:has-text("Create Snapshot")');
