...
test.describe("Batch Authorization Testing", () => {
  ...
  test("should display batch testing interface", async ({ page }) => {
    ...
    await expect(
      page.locator('button:has-text("User Access Tests")'),
    ).toBeVisible();
    await expect(
      page.locator('button:has-text("Role-Based Tests")'),
    ).toBeVisible();
  });
  ...
  test("should show test status icons", async ({ page }) => {
    ...
    const hasIcons =
      (await successIcon.count()) > 0 || (await failureIcon.count()) > 0;
  ...
  test("should calculate correct statistics", async ({ page }) => {
    ...
    const totalText = await page
      .locator("text=Total Tests")
      .first()
      .textContent();
    const successfulText = await page
